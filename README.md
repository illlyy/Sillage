...
Sillage 是一款为 Android 设计的 AI 编程应用，把 AI 编程需要的所有东西装进一个 APK：原生 AI 对话、Agent 任务执行、Git 项目工作区、Termux 终端、Codex WebUI、MCP/Skills 扩展、悬浮球以及可选的应用内代理。

你可以直接在移动设备上打开项目、向 AI 描述任务、实时查看执行过程、审批危险操作、检查代码变更，然后继续完成后续开发工作——整个流程不再依赖桌面电脑。
...
核心体验：一个流畅的原生聊天界面，支持流式输出、富文本 Markdown、代码高亮、表格与公式，并能在生成过程中实时渲染。
