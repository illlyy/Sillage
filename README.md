...
> Private monorepo backup of an unofficial Termux-based Android application with a vendored Codex Web integration. This project is not an official OpenAI, Termux, Mihomo, or MetaCubeX product.

## Repository layout

- The Android/Gradle project remains at the repository root.
- `vendor/codex-web/` contains a source snapshot based on `https://github.com/0xcaff/codex-web` plus the Android bridge changes.
- `app/src/main/assets/codex-desktop/` is the generated WebView snapshot currently loaded by the APK. Treat it as generated output and do not edit it by hand.
- `app/src/main/assets/mihomo/` contains pinned Mihomo and MetaCubeXD assets with license notices and checksums.

## Android identity and compatibility

The Android application ID is `com.ilyop.codex`. It is not a drop-in replacement for the official `com.termux` application. Do not install it over an official Termux installation. Termux plugins can be incompatible because package names, signing identities, and the original shared UID relationship differ. Back up `$HOME` and `$PREFIX` before changing builds or signing keys.

The repository's `app/dev_keystore.jks` is the public upstream debug key and must never be used as a production signing identity. Keep any release keystore and password outside Git and outside this repository.

## Build

1. Configure the local Android SDK in the ignored `local.properties` file.
2. Build from the repository root with `./gradlew testDebugUnitTest` and `./gradlew assembleDebug` (or `gradlew.bat` on Windows).
3. To regenerate the vendored WebView, review `vendor/codex-web/UPSTREAM.md`, then run `tools/build-codex-web-assets.ps1`. The script builds without replacing Android assets unless `-Apply` is passed.
...
- Before an SDK, Gradle, or AGP upgrade, create and push a tag such as `backup/pre-sdk-upgrade-YYYY-MM-DD`.
- Perform each major upgrade on a dedicated branch such as `upgrade/android-sdk-35`.
- Never use `git add .` without reviewing `git status`; `.tmp/`, logs, APKs, local SDK configuration, credentials, proxy subscriptions, and production signing material must remain untracked.
- Use `tools/verify-private-backup.ps1` before each push.

## Security

Codex Web and the Mihomo controller are intended to remain loopback-only. Anyone who can reach an unauthenticated Codex Web endpoint may be able to execute commands, read files, access credentials, or consume the signed-in Codex account. Do not bind these services to `0.0.0.0` or expose them directly through a public reverse proxy. Use an authenticated tunnel such as SSH, WireGuard, or Tailscale if remote access is required.

Provider API keys, Mihomo subscription URLs, proxy credentials, bearer tokens, and GitHub credentials must never be committed. The current application stores provider configuration in private app preferences; hardening this with Android Keystore-backed encryption remains recommended before public distribution.

## Licensing

The Android base remains GPLv3-only under `LICENSE.md`, with the existing Termux module exceptions. Vendored third-party information is summarized in `THIRD_PARTY_NOTICES.md`. The generated Codex Desktop snapshot is retained only for private reproducibility and must be reviewed before any public publication.
