...
//#endregion
//#region src/browser/routes.ts
function mapBrowserPathToInitialRoute(pathname, search) {
	if (pathname === "/share/receive" && search) {
		...
		return {
			memoryPath: prompt ? `/?${new URLSearchParams({ prompt }).toString()}` : "/",
			browserPath: "/"
		};
	}
	return { memoryPath: mapBrowserPathToRoute(pathname) };
}
function mapBrowserPathToRoute(pathname) {
...
function mapMemoryPathToBrowserPath(pathname) {
	if (pathname === "/") return {
		path: "/",
		titleChange: "Codex"
	};
...
function dispatchNavigateToRoute(path) {
...
//#endregion
//#region src/browser/files.ts
function openBrowserFilePicker({ allowMultiple, imagesOnly }) {
...
async function uploadFiles(files) {
	...
	const native = window.CodexDesktopNative;
...
function decodeSharedFile(file) {
	...
	return new File([bytes], file.name || "shared-file", { type: file.type || "application/octet-stream" });
}
function composerDropTarget() {
	const focused = document.activeElement;
	if (focused instanceof HTMLElement && focused.isContentEditable) return focused;
	return document.querySelector("[contenteditable=\"true\"], textarea[data-composer-navigation-target], textarea");
}
async function importAndroidShare(payloadJson) {
	const native = window.CodexDesktopNative;
	if (!native?.readSharedFiles) return false;
	const decoded = JSON.parse(native.readSharedFiles(payloadJson));
...
async function handleLocalFilePickerMessage(message) {
	try {
		sendFetchResponse(message, {
			responseType: "success",
			body: await handleLocalFilePickerMessageInner(message)
		});
	} catch (error) {
		console.error(error);
		sendFetchResponse(message, {
...
async function handleLocalFilePickerMessageInner(message) {
	const request = parsePickFilesRequest(message);
	const allowMultiple = message.url === "vscode://codex/pick-files";
	const uploadedFiles = await uploadFiles(await openBrowserFilePicker({
		allowMultiple,
		imagesOnly: request.imagesOnly
	}));
	return allowMultiple ? { files: uploadedFiles } : { file: uploadedFiles[0] ?? null };
}
function isCodexFetchMessage(value) {
	return isRecord(value) && value.type === "fetch";
}
function isLocalFilePickerMessage(value) {
	return isCodexFetchMessage(value) && value.method.toUpperCase() === "POST" && (value.url === "vscode://codex/pick-files" || value.url === "vscode://codex/pick-file");
}
function parsePickFilesRequest(message) {
	if (!message.body) return {};
	try {
		const parsed = JSON.parse(message.body);
		if (!isRecord(parsed)) return {};
		return {
			imagesOnly: typeof parsed.imagesOnly === "boolean" ? parsed.imagesOnly : void 0,
			pickerTitle: typeof parsed.pickerTitle === "string" ? parsed.pickerTitle : void 0
		};
	} catch {
		return {};
	}
}
function sendFetchResponse(message, response) {
	emitRendererEvent("codex_desktop:message-for-view", [response.responseType === "success" ? {
		type: "fetch-response",
		responseType: "success",
...
//#endregion
//#region node_modules/@tanstack/query-core/build/modern/index.js
var init_modern$1 = __esmMin((() => {
	init_environmentManager();
	init_notifyManager();
	init_queryClient();
	init_queryObserver();
...
//#endregion
//#region node_modules/@tanstack/react-query/build/modern/types.js
...
var import_react$5, import_jsx_runtime$5, QueryClientContext, useQueryClient, QueryClientProvider;
var init_QueryClientProvider = __esmMin((() => {
	...
	QueryClientContext = import_react$5.createContext(void 0);
	useQueryClient = (queryClient) => {
		const client = import_react$5.useContext(QueryClientContext);
	...
	QueryClientProvider = ({ client, children }) => {
		...
		return /* @__PURE__ */ (0, import_jsx_runtime$5.jsx)(QueryClientContext.Provider, {
			value: client,
			children
		});
	};
})), import_react$4, IsRestoringContext, useIsRestoring;
var init_IsRestoringProvider = __esmMin((() => {
	import_react$4 = /* @__PURE__ */ __toESM(require_react(), 1);
	IsRestoringContext = import_react$4.createContext(false);
	useIsRestoring = () => import_react$4.useContext(IsRestoringContext);
...
var import_react$3, QueryErrorResetBoundaryContext, useQueryErrorResetBoundary;
var init_QueryErrorResetBoundary = __esmMin((() => {
	import_react$3 = /* @__PURE__ */ __toESM(require_react(), 1);
	require_jsx_runtime();
	QueryErrorResetBoundaryContext = import_react$3.createContext(createValue());
	useQueryErrorResetBoundary = () => import_react$3.useContext(QueryErrorResetBoundaryContext);
}));
//#endregion
//#region node_modules/@tanstack/react-query/build/modern/errorBoundaryUtils.js
var import_react$2, ensurePreventErrorBoundaryRetry, useClearResetErrorBoundary, getHasError;
var init_errorBoundaryUtils = __esmMin((() => {
	import_react$2 = /* @__PURE__ */ __toESM(require_react(), 1);
	init_modern$1();
	ensurePreventErrorBoundaryRetry = (options, errorResetBoundary, query) => {
	...
	getHasError = ({ result, errorResetBoundary, throwOnError, query, suspense }) => {
...
function useBaseQuery(options, Observer, queryClient) {
	...
	const [observer] = import_react$1.useState(() => new Observer(client, defaultedOptions));
	const result = observer.getOptimisticResult(defaultedOptions);
	const shouldSubscribe = !isRestoring && options.subscribed !== false;
	import_react$1.useSyncExternalStore(import_react$1.useCallback((onStoreChange) => {
...
//#endregion
//#region src/browser/icons/CloseIcon.tsx
function CloseIcon() {
	return /* @__PURE__ */ (0, import_jsx_runtime$3.jsx)("svg", {
		"aria-hidden": "true",
		className: ["icon-xs"].join(" "),
		fill: "none",
		height: "21",
		viewBox: "0 0 21 21",
		width: "21",
...
var init_CloseIcon = __esmMin((() => {
	import_jsx_runtime$3 = require_jsx_runtime();
}));
//#endregion
//#region src/browser/icons/FolderIcon.tsx
function FolderIcon() {
...
//#endregion
//#region src/browser/icons/UpIcon.tsx
function UpIcon() {
	return /* @__PURE__ */ (0, import_jsx_runtime$1.jsx)("svg", {
		"aria-hidden": "true",
		className: ["icon-2xs"].join(" "),
		fill: "none",
		height: "20",
		viewBox: "0 0 20 20",
		width: "20",
...
var init_UpIcon = __esmMin((() => {
	import_jsx_runtime$1 = require_jsx_runtime();
}));
//#endregion
//#region src/browser/icons/index.ts
...
//#endregion
//#region src/browser/workspace-root-dialog.tsx
function errorMessage(error) {
	if (error instanceof Error) return error.message;
	return String(error);
}
function WorkspaceRootDialog({ listDirectory, onClose }) {
	const [directoryPath, setDirectoryPath] = (0, import_react.useState)(null);
	const [userSelectedPath, setUserSelectedPath] = (0, import_react.useState)(null);
	...
	const entries = (0, import_react.useMemo)(() => directoryQuery.data?.entries.filter((entry) => entry.type === "directory") ?? [], [directoryQuery.data?.entries]);
	...
	const queryError = directoryQuery.isError ? errorMessage(directoryQuery.error) : null;
	function navigateTo(nextDirectoryPath) {
	...
	function handleSubmit(event) {
		event.preventDefault();
		if (selectedPath && !isBusy) onClose(selectedPath);
	}
						...
						children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
							...
							children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
								...
								children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
									...
									className: ["sr-only"].join(" "),
									id: DESCRIPTION_ID,
									children: "选择 Termux 本地文件夹作为项目。"
								})]
							})
						})
					}),
								...
								children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
									...
									children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
										...
										children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
											...
											type: "button",
											children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(UpIcon, {})
											...
											children: isLoading ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
												...
												return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
													className: [
														...
														entry.path === selectedPath ? "bg-token-list-hover-background" : ""
													].filter(Boolean).join(" "),
													...
													title: entry.path,
													type: "button",
													children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(FolderIcon, {}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
														className: ["truncate"].join(" "),
														children: entry.name
													})]
												}, entry.path);
											})
...
async function openSelectWorkspaceRootDialog({ listDirectory }) {
	...
	const reactRoot = (0, import_client.createRoot)(ensureHost());
	reactRoot.render(/* @__PURE__ */ (0, import_jsx_runtime.jsx)(QueryClientProvider, {
		client: queryClient,
		children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(WorkspaceRootDialog, {
			listDirectory,
			onClose: resolveFn
		})
	}));
...
var import_react, import_client, import_jsx_runtime, TITLE_ID, DESCRIPTION_ID;
var init_workspace_root_dialog = __esmMin((() => {
	init_modern();
	import_react = /* @__PURE__ */ __toESM(require_react());
	import_client = require_client();
	init_icons();
	import_jsx_runtime = require_jsx_runtime();
	TITLE_ID = "codex-web-workspace-root-dialog-title";
	DESCRIPTION_ID = "codex-web-workspace-root-dialog-description";
}));
//#endregion
//#region src/browser/shim.ts
var shim_exports = /* @__PURE__ */ __exportAll({
	contextBridge: () => contextBridge,
	emitRendererEvent: () => emitRendererEvent,
...
function emitRendererEvent(channel, args) {
...
function ensureSocket() {
	if (window.CodexDesktopNative) {
		window.__codexDesktopReceive ??= (incoming) => {
			try {
				handleIncomingMessage(typeof incoming === "string" ? JSON.parse(incoming) : incoming);
			} catch (error) {
				console.error("[android-bridge] failed to parse native IPC message", error);
			}
		};
		window.__codexDesktopReceiveBatch ??= (incomingBatch) => {
			if (!Array.isArray(incomingBatch)) return;
			for (const incoming of incomingBatch) window.__codexDesktopReceive(incoming);
		};
		return;
	}
...
var RECONNECT_DELAY_MS, requestCounter, socket, reconnectTimeoutId, outboundQueue, pendingInvokes, pendingDirectoryEntries, rendererListeners, themeMediaQuery, mobileMediaQuery, initialSidebarState, electronShim, buildFlavor, initialRoute, ipcRenderer, contextBridge, webUtils;
	...
	ipcRenderer = {
		invoke(channel, ...args) {
			if (channel === "codex_desktop:message-from-view" && args.length === 1) {
				if (isOpenInBrowserMessage(args[0])) window.open(args[0].url, "_blank", "noopener,noreferrer");
				if (isLocalFilePickerMessage(args[0])) return handleLocalFilePickerMessage(args[0]);
				if (isUnhandledAddWorkspaceRootOptionMessage(args[0])) return openSelectWorkspaceRootDialog({ listDirectory: requestWorkspaceDirectoryEntries }).then((root) => {
					if (!root) return;
					return invokeMain(channel, [{
						...args[0],
						root
					}]);
				});
		...
		sendSync(channel, ..._args) {
			if (channel === "codex_desktop:get-sentry-init-options") return {
			...
			if (channel === "codex_desktop:get-system-theme-variant") return themeMediaQuery.matches ? "dark" : "light";
			return unimplemented("ipcRenderer.sendSync");
		}
	};
	ensureSocket();
	contextBridge = { exposeInMainWorld(_key, _api) {
		Reflect.set(window, _key, _api);
	} };
	webUtils = { getPathForFile(_file) {
